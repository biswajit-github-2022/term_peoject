...
class DiffusionUnet(nn.Module):
    ...
    def forward(self, x: torch.Tensor, step: tp.Union[int, torch.Tensor], condition: tp.Optional[torch.Tensor] = None):
        ...
        if self.use_codec:              assert condition is not None, "Model defined for conditionnal generation"
            condition_emb = self.conv_codec(condition)              assert condition_emb.size(-1) <= 2 * z.size(-1), \
            ...
            if not self.cross_attention:
                ...
                cross_attention_src = condition_emb.permute(0, 2, 1)                  B, T, C = cross_attention_src.shape
