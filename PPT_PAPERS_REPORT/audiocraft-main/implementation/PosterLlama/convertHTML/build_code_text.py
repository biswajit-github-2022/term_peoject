...
class CustomDataLoader(DataLoader):  
    def __init__(
        ...
        **kwargs
        ...
        self.bbox_quantization = bbox_quantization          self.consistency_num = args.consistency_num
    ...
    def convert_num_to_html(self, coord_lst=None, category_lst=None, self_consistency=False, consistency_num=10):
            batched_html_lst = []          batched_cond_cate, batched_cond_bbox = [], []          unconditional_ans=[]
        ...
        cond_cate_to_size_pos, cond_cate_size_to_pos, cond_cate_pos_to_size = [], [], [] 
                ...
                for coord, category in zip(coords, categories):
                    ...
                    x, y = int(coord[0] - w / 2), int(coord[1] - h / 2)                     real_category = self.category_map[category]
                ...
                if self_consistency == True:                      shuffle_lst = [i for i in range(len(html_content))]
                    ...
                    for random_order in shuffled_results:
                        ...
                        cond_cate_to_size_pos.append("\n".join(new_size_pos_mask_html))                         cond_cate_size_to_pos.append("\n".join(new_pos_mask_html))                         cond_cate_pos_to_size.append("\n".join(new_size_mask_html))                         random_mask.append("\n".join(new_random_mask_html)) 
    ...
    def custom_function(self, data, id_, self_consistency=True, consistency_num=10): 
        ...
        bbox_real, _ = to_dense_batch(data.x, data.batch)                  text = data.text
        text = text[0]
                W, H,name = data.attr["width"], data.attr["height"],data.attr["name"]         self.W=W[0].item()
        self.H=H[0].item()
        ...
        real_idx = size_ * bbox_real         if self.bbox_quantization == "code":
            ...
            bbox_lst = self.filter_invalid_num(real_idx, mask)                  preposed_res = self.convert_num_to_html(bbox_lst, label_lst, self_consistency=self_consistency, consistency_num=consistency_num)
