...
class CustomDataLoader(DataLoader):  
    def __init__(
        ...
        **kwargs
        ...
        if args.infilling:
            self.cond_cate_to_size_pos = INFILLING_INSTRUCTION.get("cond_cate_to_size_pos")             self.cond_cate_size_to_pos = INFILLING_INSTRUCTION.get("cond_cate_size_to_pos")
        ...
        if args.add_task_instruction:
            task_instruction = TASK_INSTRUCTION[args.dataset_name]             self.cond_cate_to_size_pos = task_instruction + self.cond_cate_to_size_pos 
        ...
        self.category_map = DATASET_META[dataset.dataset_name]         self.glue_template_train_eval = SEP_SEQ[0]
        self.glue_template_test = SEP_SEQ[1]
        self.glue_template_codegen_train = SEP_SEQ[2]
        self.glue_template_codegen_test = SEP_SEQ[3]         
        self.tokenizer = tokenizer         self.N_category = dataset.N_category         self.bbox_quantization = bbox_quantization          self.consistency_num = args.consistency_num
    ...
    def build_input_with_ele_dict(self, ele_dict: Dict, type=None):
        ...
        if type == "html_content":
            ...
            random_mask_num = random.choice([3, 4])             selected_mask_element = random.sample(['x', 'y', 'w', 'h'], random_mask_num)
            answer_notepad = []
    ...
    def convert_num_to_html(self, coord_lst=None, category_lst=None, self_consistency=False, consistency_num=10):
        batched_html_lst = []          batched_cond_cate, batched_cond_bbox = [], []          cond_cate_to_size_pos, cond_cate_size_to_pos = [], [] 
        random_mask = []
    
        if coord_lst is not None and category_lst is not None:             for coords, categories in zip(coord_lst, category_lst):
                ...
                if self_consistency == True:                      shuffle_lst = [i for i in range(len(html_content))]
    ...
    def build_random_mask(self, lst):
        new_lst = lst.copy()
        num = random.sample([3, 4], 1)[0]          pos = random.sample([0,1,2,3], num)
        for i in pos:
    ...
    def custom_function(self, data, id_, self_consistency=True, consistency_num=10):  
        label, mask = to_dense_batch(data.y, data.batch)           bbox_real, _ = to_dense_batch(data.x, data.batch)                                  W, H,name = data.attr["width"], data.attr["height"],data.attr["name"]         size_ = torch.cat((W.unsqueeze(-1), H.unsqueeze(-1), W.unsqueeze(-1), H.unsqueeze(-1)), dim=-1)
        size_ = size_.unsqueeze(1)
        real_idx = size_ * bbox_real         if self.bbox_quantization == "code":
            label = label.to(torch.int).tolist()
            label_lst = self.filter_invalid_num(label, mask)             real_idx = real_idx.to(torch.float).tolist()
            
            real_idx = round_nested_list(real_idx, 1)
            bbox_lst = self.filter_invalid_num(real_idx, mask)             preposed_res = self.convert_num_to_html(
        ...
        if self_consistency:              W = W.repeat(len(batched_html_lst))
        ...
        if self.infilling and self.split in ("train", "val"):              cond_cate_to_size_pos_golden = [f" {SEP_TOKEN} ".join(f"{key} {value}" for key, value in cond_cate_to_size_pos_res_dict.items())]
