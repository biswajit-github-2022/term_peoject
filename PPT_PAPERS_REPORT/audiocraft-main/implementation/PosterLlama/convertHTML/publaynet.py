...
class PubLayNet(BaseDataset):
    ...
    def __init__(self, datapath, split='train', transform=None):
        super().__init__(datapath, split, transform)
        self.N_category = self.num_classes
        self.max_box_bins = 32
    ...
    def process(self):
        for split_publaynet in ['train', 'val']:
            ...
            for img_id in sorted(coco.getImgIds()):
                ...
                N = len(elements)
                if N == 0 or self.max_bbox_bins < N:                      continue
                ...
                boxes = torch.tensor(boxes, dtype=torch.float)                  labels = torch.tensor(labels, dtype=torch.long)
