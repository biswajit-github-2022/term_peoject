...
class PKUDataset(BaseDataset):
    ...
    def __init__(self,dir,split,max_seq_length,transform=None):
        ...
        self.img = [item for item in self.img if '1739' not in item]         self.poster_name = list(map(lambda x: "train/" + x.replace("_mask", ""), self.img))
    ...
    def process(self):
        ...
        indices = torch.randperm(len(data_list),generator=generator)         data_list = [data_list[i] for i in indices]
        N = len(data_list)
