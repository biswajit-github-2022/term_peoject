...
def compute_iou(
    ...
    l1, t1, r1, b1 = convert_xywh_to_ltrb(box_1.T)     l2, t2, r2, b2 = convert_xywh_to_ltrb(box_2.T)
...
def __compute_average_iou(layout: Layout, perceptual: bool = False) -> float:
    ...
    ii, jj = np.meshgrid(range(N), range(N))
    ii, jj = ii.flatten(), jj.flatten()
    is_non_diag = ii != jj      ii, jj = ii[is_non_diag], jj[is_non_diag]
    ...
    if perceptual:
        ...
        cond = iou > np.finfo(np.float32).eps          if len(iou[cond]) > 0:
