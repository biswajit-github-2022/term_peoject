...
class LayoutFID():
    def __init__(self, pth, device='cpu'):
        num_label = 13         self.model = LayoutNet(num_label).to(device)
...
class LayoutNet(nn.Module):
    ...
    def extract_features(self, bbox, label, padding_mask, label_idx_replace=False, label_idx_replace_2=False):
        b = self.fc_bbox(bbox)
        if label_idx_replace_2:             label[label==2] = 3             label[label==3] = 444             label[label==444] = 4             label[label==1] = 2         else:             label[label==1] = 3             label[label==4] = 3             label[label==3] = 444             label[label==5] = 2             label[label==444] = 4             label[label==2] = 2 
...
def preprocess_batch(layouts, max_len: int):
    ...
    empty_ids = []      for sample in layouts:
        ...
        if pad_len == max_len:
            ...
            empty_ids.append(1)              pad_bbox = torch.tensor(
