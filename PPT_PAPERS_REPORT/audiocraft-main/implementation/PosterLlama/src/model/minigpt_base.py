...
class MiniGPTBase(BaseModel):
    ...
    def __init__(
        ...
        prompt_template="",
        end_sym='\n',
        low_resource=False,          device_8bit=0,          lora_r=0,          lora_target_modules=["q_proj", "v_proj"],
        lora_alpha=16,
        lora_dropout=0.05,
    ):
        super().__init__()
    ...
    def preparing_embedding(self,samples):
        ...
        regress_token_ids = regress_tokens.input_ids         regress_atts = regress_tokens.attention_mask         part_targets = regress_token_ids.masked_fill(
    ...
    def embed_tokens(self, token_ids):
        if hasattr(self.llama_model.base_model, 'model'):             embeds = self.llama_model.base_model.model.model.embed_tokens(token_ids)
        else:
    ...
    def get_context_emb(self, prompt, img_list):
        ...
        seg_tokens = [
            self.llama_tokenizer(
                seg, return_tensors="pt", add_special_tokens=i==0).to(device).input_ids             for i, seg in enumerate(prompt_segs)
        ]
    ...
    def generate(
        ...
        for output_token in outputs:
            ...
            output_texts = output_texts.split('</s>')[0]              output_texts = output_texts.replace("<s>", "")
