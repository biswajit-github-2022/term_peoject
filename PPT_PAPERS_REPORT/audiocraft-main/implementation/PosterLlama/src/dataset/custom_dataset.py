...
class RawFileDataset(Dataset):  
    def __init__(self, file,img_file_path ="data/cgl_dataset/augment_cgl",image_processor = Blip2ImageEvalProcessor(),vit_model_name = None,aug=False):  
        ...
        self.sample_prob = [1, 1, 1, 2, 2, 2, 2]          self.img_file_path = img_file_path         self.image_processor = image_processor
    ...
    def __getitem__(self, index):
        ...
        completion_seq_modeling = data["completion_seq_modeling"]        refinement_seq_modeling = data["refinement_seq_modeling"]        name = process_input(data["name"],aug=self.aug)         
        ...
        selected_types = random.choices(self.cond_type, self.sample_prob, k=1)          instance = instances[selected_types[0]]
        return instance