...
def metrics_rea(img_names, clses, boxes):
    ...
    for idx, name in enumerate(img_names):
        ...
        img_g_xy = np.array(img_to_g_xy(pic)) / 255         cal_mask = np.zeros_like(img_g_xy)
        ...
        total_area = np.sum(cal_mask)         total_grad = np.sum(img_g_xy[cal_mask == 1])         if total_grad and total_area:
...
def main():
    no = 1
    save_dir = f"log_dir/DS_GAN/CGL-Dataset/generated_sample"
    log_dir = "log_dir/train_stage2_with_augment_dino_codellama/generated_sample/16"
    print(log_dir)
    img_path = os.path.join(log_dir,"text_order.json")
    
    clx_path = os.path.join(log_dir,"clses.json")    clx_gt_path = os.path.join(log_dir,"clses_gt.json")
    box_path = os.path.join(log_dir,"box.json")    box_gt_path = os.path.join(log_dir,"box_gt.json") 
